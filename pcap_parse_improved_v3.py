...
def main():
    ...
    write_excel_row(sheet,0,['PKT NUMBER','TIMESTAMP','VLAN','ETH SIZE','lost?','acum burst size', 'added delay', 'burst size'])
    ...
    for ts, packet in dpkt.pcap.Reader(open(args.pcap1,'rb')):
        ...
        if packet_found == 0:
            # the packet is lost
            # add the vaule 'Lost' in the corresponding column
            excel_entry.append('Lost')
            # add the acum size of the burst of lost packets
            excel_entry.append(lost_burst_size+1)
        else:
            # the packet has arrived correctly
            # add the vaule 'Arrived' in the corresponding column
            excel_entry.append('Arrived')
